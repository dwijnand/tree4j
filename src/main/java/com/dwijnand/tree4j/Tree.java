...
/**
 * A tree-like data structure, specifically an arborescence: a rooted, directed,
 * acyclic graph, with all edges pointing away from the root, and in which any
 * two nodes are connected by maximum one directed path.
 * <p/>
 ...
 * contained in the tree, is passed to {@link #getParent(Object)} or
 * {@link #getChildren(Object)}.
 * <p/>
 * This interface only defines querying methods, so it <em>may</em> be used as
 * the basis of an immutable object. See {@link ImmutableTree} for an extension
 * to this interface that adds the immutability restriction and
 * {@link MutableTree} for an extension that adds state-mutating methods.
 *
...
public interface Tree<T> extends Iterable<Map.Entry<T, T>> {
     ...
     * The order of the children within the collection depend on the
